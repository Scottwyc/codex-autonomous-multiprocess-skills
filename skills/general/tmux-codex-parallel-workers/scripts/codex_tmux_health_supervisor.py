...
def classify_capture(text: str, patterns: list[re.Pattern[str]]) -> dict[str, Any]:
...
def supervise_target(
    ...
    health = classify_capture(captured, patterns)
    target_status.update(
        {
            "state": "stuck-error" if health.get("stuck") else "ok",
...
def supervise_once(base: Path, args: argparse.Namespace, loop_state: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for target_info in targets:
        statuses.append(supervise_target(base, target_info, args, patterns, loop_state))
    return statuses
