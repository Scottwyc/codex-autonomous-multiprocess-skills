...
description: 'Use when a project needs long-running autonomous follow-up: keep the current session alive, launch and monitor experiments or jobs over time, balance GPU/CPU resources, maintain unified coordinator constraints for child processes, compact coordinator memory to protect context budget, maintain a user-facing clickable key-results dashboard, update project docs continuously, default to tmux-launched Codex worker parallelism for non-blocking branch tasks, use branch-manager worker hierarchies for major experimental branches unless disabled or unsuitable, and derive project-specific skills from this template when a specialized workflow is needed.'
---
...
6. Update documents continuously.
   ...
   - Keep temporary control documents bounded and replaceable. Current-state constraints/schedule/context/consult/recovery-handoff files should contain only currently effective rules, active workers, current decisions, recent terminal summaries, and evidence pointers; full historical detail belongs in append-only event files, worker reports, or timestamped archives.
   - Represent each active worker once in compact control-plane tables. Do not duplicate per-worker task text, key-file inventories, resource blocks, progress/report tails, job lists, status, logs, or captures across schedule, consult, memory, and recovery views; read exact worker evidence only when a decision requires it.
   - Treat `COORDINATOR_CONSTRAINTS.md` as a live current-state contract, not an append-only policy history. When old main targets, resource owners, task authorizations, or overrides are superseded, rewrite a concise current version with `constraints --set-file`; the manager archives the previous live version automatically.
...
### 4a. User-Facing Key Results Dashboard
...
3. Write or refresh `.codex/tmux-workers/COORDINATOR_CONSTRAINTS.md` before launching workers. This file is the unified constraint contract that every worker must load before its task prompt.
   ...
   - Use `constraints --set-file <concise-current.md>` when accumulated updates contain superseded or conflicting rules. Keep only currently effective authority, ownership, permissions, queues, and fail-closed boundaries in the live file; the previous live version is archived automatically under `archive/constraints/`.
...
4. If the main coordinator itself is running inside tmux, register it with `register-coordinator --target <SESSION:WINDOW.PANE> --cwd "$PWD"` before long autonomous follow-up. This gives future sessions a durable `COORDINATOR_RECOVERY.md` handoff and lets the health supervisor restart the coordinator after context-window exhaustion.
   - Keep the registered main and managed worker window names stable. The manager disables tmux automatic/application window renaming at registration/creation and reapplies the lock during in-place recovery; do not re-enable it, because name drift can make a live pane appear missing and break notification/recovery routing.
...
See [references/workflow.md](references/workflow.md) for the reusable operating procedure, monitoring cadence, documentation pattern, and derivation checklist. See [references/user-key-results-dashboard.md](references/user-key-results-dashboard.md) for the user-facing dashboard schema and template.
