...
If the main coordinator itself is running inside tmux, it can be registered with `register-coordinator`. The manager locks the registered window name with `automatic-rename=off` and `allow-rename=off`, applies the same lock to managed worker windows, and reapplies it during in-place recovery. The health supervisor can then detect `Codex ran out of room in the model's context window`, run `recover-coordinator`, and respawn the exact registered pane in place while handing the fresh thread `COORDINATOR_RECOVERY.md` plus the existing worker registry, schedule, progress/report files, jobs, peer messages, branch summaries, and consultation context. This stable-target policy prevents coordinator-registration drift and dual-main sessions. Replacement of a confirmed-missing coordinator target remains an explicit `--restart-main-when-missing` opt-in.
